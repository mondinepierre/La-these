...
<div className="my-6 py-4 border-y border-stone-border"><span className="block text-center text-2xl font-display italic text-gold tracking-tight">ROIIC = ΔNOPAT / ΔCapital investi</span></div>

- **ΔNOPAT** : la variation du résultat opérationnel net d'impôts entre deux périodes
- **ΔCapital investi** : la variation du capital investi entre ces mêmes périodes
...
```
ROIIC (année N) = (NOPAT N − NOPAT N-1) / (Capital investi N-1 − Capital investi N-2)
```
...
C'est cette question — la durabilité du spread — qui ouvre la voie à la valorisation rigoureuse. Dans le prochain article, nous verrons comment ajuster le PER historique à l'environnement de taux pour ne pas comparer des multiples de 2021 avec ceux d'aujourd'hui.
