...
import type { Secteur, ZoneGeo, Conviction } from '@/types/analyses'
...
export function AnalysesIndex({ analyses }: Props) {
  ...
  // Listes dynamiques — construites depuis les données réelles
  const secteurs = useMemo(
    () => getUniques(analyses, (a) => a.secteur) as Secteur[],
    [analyses]
  )
  const zones = useMemo(
    () => getUniques(analyses, (a) => a.geo) as ZoneGeo[],
    [analyses]
  )
  ...
  // Séparer valeurs et ponctuelles dans les résultats filtrés
  const valeurs = filtered.filter((a) => a.type === 'valeur')
  const ponctuelles = filtered.filter((a) => a.type === 'ponctuelle')
    ...
    <main style={{ maxWidth: '860px', margin: '0 auto', padding: '3rem 1.5rem' }}>
      ...
      <div style={{ marginBottom: '2rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <FilterPill
            label="Tous"
            active={geoFilter === 'tous'}
            onClick={() => setGeoFilter('tous')}
          />
          {zones.map((z) => (
            <FilterPill
              key={z}
      ...
      {filtered.length === 0 ? (
        ...
        <>
          {/* Valeurs suivies */}
          {valeurs.length > 0 && (typeFilter === 'tous' || typeFilter === 'valeur') && (
            <section style={{ marginBottom: '2.5rem' }}>
              ...
              >
                Valeurs suivies ({valeurs.length})
              </h2>
          ...
          {ponctuelles.length > 0 && (typeFilter === 'tous' || typeFilter === 'ponctuelle') && (
            <section>
              <h2
                style={{
                  fontFamily: 'var(--font-sans)',
              ...
              >
                Analyses ponctuelles ({ponctuelles.length})
              </h2>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {ponctuelles.map((a) => (
                  <AnalyseCard key={a.slug} analyse={a} />
